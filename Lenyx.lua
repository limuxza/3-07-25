...
function Linux:PlaySound(soundId)
    ...
    spawn(function()
        local success, sound = pcall(function()
            ...
            soundInstance.SoundId = "rbxassetid://" .. tostring(soundId)
        ...
        if not success then
            warn("Erro ao reproduzir som:", sound)
        end
    end)
end
...
function Linux.Create(config)
    ...
    -- Função para animar fillbar dos sliders ao trocar de aba
    local function AnimateSliderFillBars(tabIndex)
        ...
        for _, element in pairs(Tabs[tabIndex].Elements) do
            if element.Type == "Slider" then
                ...
                if fillBar then
                    ...
                    TweenService:Create(fillBar, tween, {Size = currentSize}):Play()
                end
            end
        end
    end
    ...
    function LinuxLib.Tab(config)
        ...
        local function SelectTab()
            ...
            -- Primeiro, fade out da aba atual
            local currentTabContent = nil
            if CurrentTab and Tabs[CurrentTab] then
                currentTabContent = Tabs[CurrentTab].Content
                if currentTabContent.Visible then
                    local fadeOutTween = TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
                    TweenService:Create(currentTabContent, fadeOutTween, {BackgroundTransparency = 1}):Play()
                    
                    task.wait(0.15) -- Aguarda o fade out completar
                end
            end
            
            -- Esconde todas as abas e reseta seus estilos
            ...
            -- Fade in da nova aba com animação do container
            TabContent.BackgroundTransparency = 1
            local fadeInTween = TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
            TweenService:Create(TabContent, fadeInTween, {BackgroundTransparency = 1}):Play()
            
            -- Animação dos sliders na nova aba
            task.wait(0.1)
            AnimateSliderFillBars(tabIndex)
    ...
    task.spawn(function()
        ...
        if DefaultTab and Tabs[DefaultTab] then
            ...
            -- Animar sliders da aba padrão
            task.wait(0.2)
            AnimateSliderFillBars(DefaultTab)
        elseif tabOrder > 0 then
            ...
            if firstTab then
                ...
                -- Animar sliders da primeira aba
                task.wait(0.2)
                AnimateSliderFillBars(1)
            end
        end
    end)
